...
import os, platform, sys, pkgutil, datetime, threading, h5py, requests
...
class Ticker:
    def __init__(self, ticker_number, count, proxies):

        self.ticker = requests.get('https://www.avanza.se/_mobile/market/{0:s}/{1:d}'.format("stock", int(ticker_number)), proxies=proxies).json()

        self.number = ticker_number
        self.name = self.ticker['name']
        self.change_percent = self.ticker['changePercent']
        ...
        if 'keyRatios' in self.ticker:
            self.PE_ratio = np.NaN if 'priceEarningsRatio' not in self.ticker['keyRatios'] else self.ticker['keyRatios']['priceEarningsRatio']
        ...
        self.price_one_week = self.ticker["priceOneWeekAgo"] if "priceOneWeekAgo" in self.ticker else np.NaN
        self.price_one_month = self.ticker["priceOneMonthAgo"] if "priceOneMonthAgo" in self.ticker else np.NaN
        self.price_six_month = self.ticker["priceSixMonthsAgo"] if "priceSixMonthsAgo" in self.ticker else np.NaN
        self.price_one_year = self.ticker["priceOneYearAgo"] if "priceOneYearAgo" in self.ticker else np.NaN
...
class Ui_MainWindow(QtWidgets.QMainWindow):    
    ...
    def setupUi(self, MainWindow):
        ...
        MainWindow.setMinimumSize(QtCore.QSize(1092, 503))
        ...
        self.__create_element(self.Interface_Label_Dividends, [835, 395, 150, 22], "Interface_Label_Dividends", text="Dividends:", font=font_headline,
                              stylesheet="QLabel { color : blue; }")
        self.Interface_Button_Dividends = QtWidgets.QPushButton(self.centralwidget)
        self.__create_element(self.Interface_Button_Dividends, [935, 395, 150, 22], "Interface_Button_Dividends", text="Show calendar", font=font_headline)

        # Block: Proxy
        self.Interface_Label_Settings = QtWidgets.QLabel(self.centralwidget)
        self.__create_element(self.Interface_Label_Settings, [835, 420, 150, 22], "Interface_Label_Settings", text="Settings:", font=font_headline, stylesheet="QLabel { color : blue; }")
        self.Interface_GroupBox_Settings = QtWidgets.QGroupBox(self.centralwidget)
        self.__create_element(self.Interface_GroupBox_Settings, [830, 425, 255, 52], "Interface_GroupBox_Settings", font=font_ee)
...
''' Interface / Message Box (dividents calendar details) ''' # HERE
class Ui_MessageBox_Dividents(QtWidgets.QMessageBox):

    # Table / Sorting
    sorting, sort_table, = {}, [0, QtCore.Qt.DescendingOrder]
    ...
    def __init__(self, tickers):
        ...
        col_param = [("Date", 70), ("Company", 160), ("Amount (per share)", 130), ("Currency", 60), ("My balance", 80)]
        ...
        self.Interface_Table_MessageBox.sortItems(self.sort_table[0], self.sort_table[1])
        self.Interface_Table_MessageBox.horizontalHeader().sectionClicked.connect(self.__table_column_sort__)
...
class GUI(Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.Interface_Button_Dividends.clicked.connect(self.dividends_calendar)
    ...
    def __table_button_click__(self):
        ...
        for index, i in enumerate(self.all_data):
            if i.number == int(self.sender().text()):
                msgBox = Ui_MessageBox_Details(self.all_data[index])
                msgBox.exec_()
    ...
    def dividends_calendar(self):
        ...
        msgBox = Ui_MessageBox_Dividents(self.all_data)
        msgBox.exec_()
    ...
    def update_table_content(self):
        ...
        if not self.__check_connection__(): return

        self.message, self.all_data, self.currency_rates, self.cell_buttons_collection, balance, self.total_balance = "", [], {}, [], {"SEK": 0}, "0 SEK"

        def __update_thread__(ticker, TICKERS, all_data, currency_rates, proxies):

            if not all_data == False: # threads for DATA
                ticker_data = Ticker(int(ticker), int(TICKERS[ticker]), proxies = proxies)
                all_data.append(ticker_data)

            else: # threads for CURRENCY
                ticker_data = Ticker(int(ticker), 1, proxies = proxies)
        ...
        for ticker in self.CURRENCY_RATES_TICKERS:
            x = threading.Thread(target=__update_thread__, args=(ticker, False, False, self.currency_rates, self.proxies))
            x.start()
